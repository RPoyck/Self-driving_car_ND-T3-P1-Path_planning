...
// 								ref_vel = 29.5; // [mph]
					...
					{
						// Redefine reference state as previous's path end point //
					...
					// Fill up the rest of the the path planner after filling it with previous points, here we will always output 50 points //
					for (unsigned int i = 1; i <= 50-previous_path_x.size(); i++)
					{
						double N  = target_dist / (.02 * ref_vel / 2.24);  // 2.24 to go from [mph] to [m/s] //
						...
						// Rotate back to original after rotating it earlier //
